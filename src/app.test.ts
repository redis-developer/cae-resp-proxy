    ...
    mockRedisServer = Bun.listen({
      ...
      socket: {
        data(socket, data) {
          ...
          // Send appropriate responses based on command content
          let responses = '';
          ...
          // Handle CLIENT SETINFO commands (send OK for each)
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      socket.connect(proxy.config.listenPort, '127.0.0.1', async () => {
        try {
          ...
          // Check that stats detect the connection
          ...
          // Check that connection ID is present
          ...
          // Test sending a response to the connected client and verify response
          let receivedData = '';
